...
pipeline {
  ...
  stages {
    ...
    // ============================================
    // BUILD & PUSH - WITH PROPER TAG MANAGEMENT
    // ============================================
              ...
              buildStages[serviceName] = {
                ...
                dir(meta.path) {
                  ...
                  if (env.AWS_CREDS_EXIST == 'true' && env.CURRENT_ECR_REGISTRY?.trim()) {
                    ...
                    def versionedImage = "${env.CURRENT_ECR_REGISTRY}/${imageName}:${env.IMAGE_TAG}"
                    def latestImage = "${env.CURRENT_ECR_REGISTRY}/${imageName}:latest"
                    
                    // Store the image tags for later use in GitOps
                    env."${serviceName.toUpperCase().replace('-', '_')}_IMAGE_TAG" = versionedImage
                    env."${serviceName.toUpperCase().replace('-', '_')}_LATEST_IMAGE" = latestImage
                    
                    // DR region disabled for now
                    // def drVersionedImage = "${env.DR_ECR_REGISTRY}/${imageName}:${env.IMAGE_TAG}"
                    // def drLatestImage = "${env.DR_ECR_REGISTRY}/${imageName}:latest"

                    // Check if Dockerfile exists before building
                    if (fileExists('Dockerfile')) {
                      
                      // Use withAWS for authentication
                      withAWS(
                        ...
                        sh """
                          aws ecr describe-repositories --repository-names ${imageName} 2>/dev/null || \
                      ...
                      sh "docker build -t ${versionedImage} -t ${latestImage} ."
                      
                      // DR region tagging disabled for now
                      // sh "docker tag ${versionedImage} ${drVersionedImage}"
                      // sh "docker tag ${versionedImage} ${drLatestImage}"
                      
                      // Run Trivy scan (optional, won't fail the build)
                      sh """
                        trivy image --severity ${env.TRIVY_SEVERITY} --exit-code 0 ${versionedImage} || true
                      """
                      ...
                      echo "✅ Successfully pushed images for ${serviceName}:"
                      ...
                      withAWS(
                        ...
                        sh """
                          ...
                          docker push ${drVersionedImage} || echo 'Failed to push versioned to DR region'
                          docker push ${drLatestImage} || echo 'Failed to push latest to DR region'
                        """
                      }
                      */
                    } else {
    ...
    // ============================================
    // GITOPS MANIFEST UPDATE - WITH PROPER TAG HANDLING
    // ============================================
    ...
    stage('GitOps Manifest Update') {
      ...
      steps {
        ...
        script {
          ...
          updateKustomizeManifests(env.KUSTOMIZE_OVERLAY)
...
// ============================================
// ECR TAG VERIFICATION - NEW FUNCTION
// ============================================

def verifyECRTags() {
  echo "========================================="
  echo "VERIFYING ECR TAGS"
  echo "========================================="
  ...
  env.CHANGED_SERVICES
    ...
    .each { serviceName ->
      ...
      withAWS(region: env.AWS_REGION, credentials: env.AWS_CREDENTIALS_ID) {
        ...
        echo "Tags for ${serviceName} in ECR:"
        if (tags) {
          tags.split('\n').each { tag ->
            echo "  - ${tag}"
          }
          ...
          if (tags.contains('latest')) {
            echo "✅ ${serviceName} has :latest tag"
          } else {
            echo "⚠️ ${serviceName} is missing :latest tag"
          }
          
          if (tags.contains(env.IMAGE_TAG)) {
            echo "✅ ${serviceName} has version tag ${env.IMAGE_TAG}"
          } else {
            echo "⚠️ ${serviceName} is missing version tag ${env.IMAGE_TAG}"
          }
        } else {
          echo "  No tags found for ${serviceName}"
        }
      }
    }
...
// ============================================
// KUSTOMIZE UPDATE HELPER - NEW FUNCTION
// ============================================

def updateKustomizeManifests(String overlayPath) {
  echo "Updating kustomize manifests in ${overlayPath}"
  ...
  env.CHANGED_SERVICES
    ...
    .each { serviceName ->
      ...
      if (serviceName == 'frontend') {
        echo "Skipping frontend in kustomize (handled by S3/CloudFront)"
        return
      }
      ...
      dir(overlayPath) {
        ...
        if (!fileExists('kustomization.yaml')) {
          echo "kustomization.yaml not found in ${overlayPath}. Creating one..."
          
          // Create a basic kustomization.yaml if it doesn't exist
          sh """
...
images:
  - name: ${meta.image}
    newName: ${env.CURRENT_ECR_REGISTRY}/${env.ECR_REPOSITORY_PREFIX}/${meta.image}
...
EOF
          ...
          // Also update any references to :latest in the manifests
          sh """
            # Also update any references to :latest in the manifests
            find . -name "*.yaml" -type f -exec sed -i 's|${meta.image}:latest|${env.CURRENT_ECR_REGISTRY}/${env.ECR_REPOSITORY_PREFIX}/${meta.image}:${env.IMAGE_TAG}|g' {} \\;
          """
        }
        
        echo "✅ Updated kustomize for ${serviceName} with tag ${env.IMAGE_TAG}"
        
        // Verify the change
        sh """
          echo "Current image in kustomization:"
          grep -A 2 "images:" kustomization.yaml || echo "No images section found"
        """
      }
    }
}
